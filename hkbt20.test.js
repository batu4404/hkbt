...
expect.extend({
    toEqualPieceArray(received, argument) {
        if (received.length !== argument.length) {
            return {
                pass: false,
                message: () => `Size is same`
            };
        }

        for (let i = 0; i < received.length; i++) {
            if (received[i].value != argument[i].value) {
                return {
                    pass: false,
...
for (let testCase of shilfRowRightTestCases) {
    test('shift left a row ' + testCase.src, () => {
        expect(hkbt.shiftRowLeft(createPieceArray(testCase.src), createEmptyArray()))
...
for (let testCase of shilfRowRightTestCases) {
    test('shift right a row ' + testCase.src, () => {
        expect(hkbt.shiftRowRight(createPieceArray(testCase.src), createEmptyArray()))
        .toEqualPieceArray(createPieceArray(testCase.des));
    });
}
...
function createEmptyArray(size=4) {
    ...
    for (let i = 0; i < size; i++) { 
        pieceArray[i] = Piece.getEmptyPiece();
    }
